...
float cameraMoveTrabslationSpeed = 0.008f;
float movX=0.0f;
float movY=0.0f;
float movZ=-5.0f;
float rot = 0.0f;
int main() {
	...
	GLFWwindow *window = glfwCreateWindow(WIDTH, HEIGHT, "Modelado geometrico - Lopez Lopez Axel Dion", nullptr, nullptr);
...
-0.5f, -0.5f, 0.5f, 0.9f, 0.9f,0.9f,//Front
...
-0.5f, 0.5f, -0.5f, 0.0f, 1.0f, 0.0f,
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		//arms
		model = glm::mat4(1.0f);
		model = glm::scale(model, glm::vec3(3.0f, 0.25f, 0.25f));
		...
		model = glm::translate(model, glm::vec3(4.0f, 1.55f, 0.0f));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		glDrawArrays(GL_TRIANGLES, 0, 36);
		...
		model = glm::mat4(1.0f);
		model = glm::scale(model, glm::vec3(1.15f, 1.0f, 1.15f));
		...
		model = glm::translate(model, glm::vec3(2.0f, 8.0f, 3.1f));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		glDrawArrays(GL_TRIANGLES, 69, 107);
		...
		model = glm::mat4(1.0f);
		model = glm::scale(model, glm::vec3(0.25f, 0.25f, 0.25f));
		model = glm::translate(model, glm::vec3(0.0f, 7.5f, 3.1f));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		glDrawArrays(GL_TRIANGLES, 0, 36);


		model = glm::mat4(1.0f);
		model = glm::scale(model, glm::vec3(0.125f, 0.125f, 0.25f));
		model = glm::translate(model, glm::vec3(0.0f, 15.0f, 4.1f));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		glDrawArrays(GL_TRIANGLES, 0, 36);
		...
		model = glm::mat4(1.0f);
		model = glm::scale(model, glm::vec3(0.8f, 1.0f, 0.0f));
 ...
 void Inputs(GLFWwindow *window) {
	 ...
	 if (glfwGetKey(window, GLFW_KEY_D) == GLFW_PRESS)
		 movX += cameraMoveTrabslationSpeed;
	 if (glfwGetKey(window, GLFW_KEY_A) == GLFW_PRESS)
		 movX -= cameraMoveTrabslationSpeed;
	 if (glfwGetKey(window, GLFW_KEY_SPACE) == GLFW_PRESS)
		 movY -= cameraMoveTrabslationSpeed;
	 if (glfwGetKey(window, GLFW_KEY_LEFT_SHIFT) == GLFW_PRESS)
		 movY += cameraMoveTrabslationSpeed;
	 if (glfwGetKey(window, GLFW_KEY_W) == GLFW_PRESS)
		 movZ -= cameraMoveTrabslationSpeed;
